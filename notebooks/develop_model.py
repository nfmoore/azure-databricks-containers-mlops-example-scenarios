...
kaggle.api.dataset_download_files('pavansubhasht/ibm-hr-analytics-attrition-dataset', path='/tmp', unzip=True)
...
columns_categorical = ["BusinessTravel", "Department", "EducationField", "Gender", "JobRole", "MaritalStatus", "Over18", "OverTime"]
...
df_attrition["Over18"] = df_attrition["Over18"].replace({"Y": "Yes", "N": "No"})

# Change data types of features
df_attrition[columns_target] = df_attrition[columns_target].replace({"Yes": 1, "No": 0}).astype("str")
df_attrition[columns_categorical] = df_attrition[columns_categorical].astype("str")
df_attrition[columns_numeric] = df_attrition[columns_numeric].astype("float")

# Split into train and test datasets
X_train, X_test, y_train, y_test = train_test_split(df_attrition[columns_categorical + columns_numeric], df_attrition[columns_target], test_size=0.20, random_state=2022)
...
with mlflow.start_run(run_name="attrition-drift") as run:
    ...
    categories_per_feature = {0:None, 1:None, 2:None, 3:None, 4:None, 5:None, 6:None, 7:None}
    drift_model = make_drift_model(X_train[drift_column_names].values, categories_per_feature)
    ...
    drift_output = drift_model.predict(X_test[drift_column_names].values, drift_type="feature", return_p_val=True, return_distance=True)

    # Display drift output
    pprint(process_drift_output(drift_output, drift_column_names), width=120, compact=True)
...
with mlflow.start_run(run_name="attrition-outlier") as run:
    ...
    outlier_model = make_outlier_model(X_train[columns_numeric].values, threshold=5)
    ...
    pprint(process_outlier_output(outlier_output, columns_numeric), width=120, compact=True)
...
class AttritionModel(mlflow.pyfunc.PythonModel):
    ...
    def load_context(self, context):
        self.classifier = joblib.load(os.path.join(context.artifacts["artifacts_path"], "classifier/model/model.pkl"))
        self.drift_model = load_detector(os.path.join(context.artifacts["artifacts_path"], "drift"))
        self.outier_model = load_detector(os.path.join(context.artifacts["artifacts_path"], "outlier"))
    ...
    def predict(self, context, model_input):
        ...
        drift_output = self.drift_model.predict(model_input[drift_column_names].values, drift_type="feature", return_p_val=True, return_distance=True)
        outlier_output = self.outier_model.predict(model_input[columns_numeric].values)
...
with mlflow.start_run(run_name="attrition-model-artifact") as run:
    ...
    y_pred_proba = joblib.load("/tmp/models/classifier/model/model.pkl").predict_proba(X_test)
